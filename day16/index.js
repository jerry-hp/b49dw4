...
app.post("/edit-blog/:id", upload.single("upload-image"), editProject);
...
async function home(req, res) {
  try {
    const query = `SELECT db_projects.id, title,start_date, end_date, duration, content, nodejs, reactjs, nextjs, typescript, image, db_projects."createdAt", db_projects."updatedAt",
    users.name AS author FROM db_projects LEFT JOIN users ON db_projects.author=users.id;`;
...
async function addProject(req, res) {
  try {
    ...
    await sequelize.query(
      `INSERT INTO db_projects(title,author, start_date, end_date, duration, content, nodejs, reactjs, nextjs, typescript, image, "createdAt", "updatedAt") 
      VALUES ('${title}',${author}, '${startdate}', '${enddate}', '${Duration}','${content}', '${nodejs}', '${reactjs}', '${nextjs}', '${typescript}', '${image}', NOW(), NOW())`
    );
...
async function editProject(req, res) {
  ...
  await sequelize.query(`UPDATE public.db_projects
    SET id=${id}, title='${title}', start_date='${startdate}', end_date='${enddate}', duration='${Duration}', content='${content}', nodejs='${nodejs}', reactjs='${reactjs}', nextjs='${nextjs}', typescript='${typescript}', image='${image}', "createdAt"=NOW(), "updatedAt"=NOW()
    WHERE id=${id};`);
...
async function blogDetail(req, res) {
  try {
    ...
    id++;
    const query = `SELECT db_projects.id, title,start_date, end_date, duration, content, nodejs, reactjs, nextjs, typescript, image, db_projects."createdAt", db_projects."updatedAt",
    users.name AS author FROM db_projects LEFT JOIN users ON db_projects.author=users.id where db_projects.id=${id};`;
    let obj = await sequelize.query(query, { type: QueryTypes.SELECT });
    console.log(obj);
